...
- **フレームストール検知**: `-progress pipe:2` の `frame=N` が60秒進まない場合もハングとみなして再起動。ALSAバッファxrun後に音声入力スレッドがブロックし、stderrへの出力自体は続く（＝従来のWatchdogタイムアウトでは検知できない）フリーズに対応
...
- **v5.1.1** (2026-08) - Watchdogにフレームストール検知を追加（ALSAバッファxrunによる音声スレッドデッドロック時、stderrは出続けるがframeが進まないフリーズを検知）
